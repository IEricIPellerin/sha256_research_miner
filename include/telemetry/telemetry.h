...
class Telemetry {
 public:
  ...
  void set_worker_state(unsigned cpu_threads, std::string cpu_extranonce2, std::string gpu_name, std::string gpu_extranonce2);
  void set_progress(std::uint64_t nonce_start, std::uint64_t nonce_next, std::uint64_t nonce_end);
 ...
 private:
  ...
  std::atomic<std::uint64_t> nonce_start_{0};
  std::atomic<std::uint64_t> nonce_next_{0};
  std::atomic<std::uint64_t> nonce_end_{0};
};
